...
export default class WechatBoy {
  ...
  async onLogin(user) {
    ...
    const contact = await this.boy.Contact.find({ name: CONTACT_NAME });
    const group = await this.boy.Room.find({ topic: ROOM_TOPIC });
    // 与好友打招呼
    // if (contact !== null) {
    //   const topNews = await Service.getTopNews();
    //   await contact.say(topNews)
    // } else {
    //   console.log('contact not found');
    // }
    if (group !== null) {
      const topNews = await Service.getTopNews();
      await group.say(topNews);
    } else {
  ...
  async onMessage (msg) {
    ...
    if (room) {
      ...
      console.log(`room: ${topic} id: ${room.id} send by: ${contact.name()} content: ${content}`);
